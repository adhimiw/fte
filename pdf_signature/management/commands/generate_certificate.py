...
class Command(BaseCommand):
    help = 'Generate a self-signed certificate for PDF signing'

    def handle(self, *args, **options):
        self.stdout.write("Generating a new self-signed certificate...")
        try:
            ...
            if cert_info:
                self.stdout.write(
                    self.style.SUCCESS('Successfully generated self-signed certificate:')
                )
                self.stdout.write(f"Certificate: {cert_info['cert_path']}")
                self.stdout.write(f"Private Key: {cert_info['key_path']}")
                self.stdout.write(f"PKCS#12 Bundle: {cert_info['p12_path']}")
                ...
                self.stdout.write(
                    self.style.ERROR('Failed to generate certificate. Check logs for details.')
                )
        except Exception as e:
            logger.exception("Error generating certificate:")
            self.stdout.write(
                self.style.ERROR(f'Error generating certificate: {str(e)}')
            )